...
app.post('/SignUp', async (req, resp) => {
    ...
    try {
        ...
        console.log(result);
        return resp.status(200).json({ message: "User registered Sucesssfully" })
    } catch (error) {
...
const fetchProduct = async () => {
    try {
        const limit = 100;
        const response = await fetch('https://dummyjson.com/products/11'); // single product by id 
...
async function addReview(product_id, comments) {
    ...
    for(const comment of comments) {
        const user_id = Math.floor(Math.random() * 6) + 9;   //  0.9
